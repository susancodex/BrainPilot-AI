    ...
    <div className="min-h-screen bg-background text-foreground antialiased overflow-x-hidden">
      ...
      <main>
        ...
        <section className="relative overflow-hidden border-b border-border">
          <div className="relative mx-auto max-w-6xl px-4 pb-16 pt-16 sm:px-6 sm:pb-24 sm:pt-20 md:px-8 md:pb-32 md:pt-28">
            <h1 className="max-w-[800px] text-3xl font-semibold leading-tight tracking-tight sm:text-4xl md:text-5xl lg:text-6xl">
              Everything you need{" "}
              <span className="text-primary">to study smarter</span>,
              in one place.
            </h1>

            <p className="mt-4 max-w-[580px] text-base leading-relaxed text-muted-foreground sm:text-lg md:text-xl">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:mt-10 sm:flex-row sm:gap-4">
              <Button size="lg" asChild className="h-12 gap-2 text-base w-full sm:w-auto">
                <Link href="/register">
                  Create free account
                  <ArrowRight className="h-5 w-5" />
                </Link>
              </Button>
              <Button size="lg" variant="outline" asChild className="h-12 text-base w-full sm:w-auto">
                <Link href="/login">Sign in to your workspace</Link>
              </Button>
            </div>

            {/* Highlight chips */}
            <ul className="mt-8 flex flex-wrap gap-x-4 gap-y-2 sm:mt-10 sm:gap-x-6 sm:gap-y-3">
              {HIGHLIGHTS.map((h) => (
                <li key={h} className="flex items-center gap-2 text-sm text-muted-foreground sm:text-base">
        ...
        <section className="border-b border-border bg-muted/50">
          <div className="mx-auto grid max-w-6xl grid-cols-2 divide-x divide-border sm:grid-cols-4 px-4 sm:px-6 sm:px-8">
            {[
              ...
              <div key={label} className="flex flex-col items-center gap-1 px-3 py-6 text-center first:pl-0 last:pr-0 sm:px-6 sm:py-8 sm:px-10">
        ...
        <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20 md:px-8 md:py-28">
          <div className="mb-12 sm:mb-16">
            <p className="mb-3 text-xs font-semibold text-primary uppercase tracking-wider sm:text-sm">What's included</p>
            <h2 className="max-w-xl text-2xl font-semibold tracking-tight sm:text-3xl md:text-4xl">
          ...
          <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map(({ icon: Icon, label, description }) => (
              <article
                key={label}
                className="group rounded-xl border border-border bg-card p-4 sm:p-6 transition-all duration-200 hover:shadow-lg hover:border-primary/30"
              >
                ...
                <p className="mt-1.5 sm:mt-2 text-sm leading-relaxed text-muted-foreground sm:text-base">{description}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="border-y border-border bg-muted/50">
          <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20 md:px-8 md:py-28">
            <div className="mb-12 sm:mb-16">
              <p className="mb-3 text-xs font-semibold text-primary uppercase tracking-wider sm:text-sm">How it works</p>
              ...
              {STEPS.map(({ number, title, body }, i) => (
                <div key={number} className="relative flex flex-col">
                  {i < STEPS.length - 1 && (
                    <div
                      aria-hidden
                      className="absolute left-[3rem] top-6 hidden h-px w-[calc(100%+2rem)] border-t border-dashed border-border sm:block sm:left-[3.5rem]"
                    />
                  )}
                  <div className="mb-3 sm:mb-4 inline-flex h-10 w-10 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-xl border border-border bg-card text-xs font-semibold text-foreground sm:text-sm">
                    {number}
                  </div>
                  <h3 className="text-base font-semibold text-foreground sm:text-lg">{title}</h3>
                  <p className="mt-1.5 sm:mt-2 text-sm leading-relaxed text-muted-foreground sm:text-base">{body}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16 md:px-8">
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-3 sm:gap-x-12 sm:gap-y-4 md:gap-x-20">
            {[
              ...
              <div key={text} className="flex items-center gap-2 text-sm text-muted-foreground sm:text-base sm:gap-3">
        ...
        <section className="border-t border-border bg-primary/5">
          <div className="mx-auto max-w-6xl px-4 py-16 text-center sm:px-6 sm:py-20 md:px-8 md:py-28">
            <h2 className="text-2xl font-semibold tracking-tight sm:text-3xl md:text-4xl">
